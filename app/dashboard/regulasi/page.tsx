...
export default function RegulasiPage() {
    ...
    const [deleteTarget, setDeleteTarget] = useState<Regulasi | null>(null);
    ...
    const { data, isLoading, mutate } = useSWR(`/api/regulasi?${params}`, fetcher);
    const items: Regulasi[] = data?.data ?? [];
    ...
    async function handleDelete() {
        ...
        setDeleting(true);
        try {
            const res = await fetch(`/api/regulasi/${deleteTarget.id}`, { method: "DELETE" });
        ...
        <>
            ...
            <main className="p-6 md:p-10 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded text-[10px] font-bold uppercase tracking-wider border border-orange-200/50">
                                Produk Hukum & Layanan
                            </span>
                        </div>
                        <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">Regulasi & Layanan</h2>
                        <p className="text-sm text-gray-500 mt-0.5">Kelola Peraturan Desa (Perdes) dan info persyaratan surat.</p>
                    </div>
                    <Link
                        href="/dashboard/regulasi/baru"
                ...
                <div className="relative max-w-md">
                    ...
                    <input
                        type="text"
                        placeholder="Cari judul atau nomor perdes..."
                        value={search}
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    {isLoading ? (
                        ...
                        <div className="text-center py-20 text-sm text-gray-400">
                            {search ? "Tidak ada yang cocok." : "Belum ada data regulasi/layanan."}
                        </div>
                    ) : (
                                        ...
                                        <tr key={r.id} className="hover:bg-gray-50/70 transition-colors">
                                            <td className="py-3 px-4">
                                                <span className={`inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border ${
                                                    ...
                                                    {r.jenis === "peraturan" ? "Perdes" : "Surat"}
                                                </span>
                                            </td>
                                            <td className="py-3 px-4">
                                                <Link href={`/dashboard/regulasi/${r.id}`} className="font-semibold text-gray-900 line-clamp-1 hover:text-teal-600 transition-colors">
                                                    {r.judul}
                                                </Link>
                                            </td>
                                            <td className="py-3 px-4 text-gray-600 text-xs">
                                                {r.jenis === "peraturan" ? (
                                                    <span>{[r.nomor, r.tahun ? `Tahun ${r.tahun}` : null, r.kategori].filter(Boolean).join(" · ") || "—"}</span>
                                                ) : "—"}
                                            </td>
                                            <td className="py-3 px-4">
                                                <div className="flex items-center justify-center" title={r.file_url ? "Ada dokumen" : "Belum ada dokumen"}>
                                                    {r.file_url ? (
                                                        <FileText className="w-4 h-4 text-emerald-500" />
                                                    ) : (
                                                        <FileX2 className="w-4 h-4 text-gray-300" />
                                                    )}
                                                </div>
                                            </td>
                                            <td className="py-3 px-4">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Link href={`/dashboard/regulasi/${r.id}`} className="p-2 rounded-lg text-gray-400 hover:text-sky-600 hover:bg-sky-50 transition-colors" title="Edit">
                                                        <Pencil className="w-4 h-4" />
                                                    </Link>
                                                    <button onClick={() => setDeleteTarget(r)} className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors cursor-pointer" title="Hapus">
                    ...
                    {!isLoading && items.length > 0 && (
                        <div className="px-4 py-3 border-t border-gray-100 bg-gray-50/40 flex items-center justify-between">
                            <span className="text-xs text-gray-400">
                                Menampilkan {(page - 1) * LIMIT + 1}–{Math.min(page * LIMIT, total)} dari {total} item
                            </span>
                ...
                <div className="fixed inset-0 z-[80] flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4 border border-gray-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-base font-bold text-gray-900">Hapus Item</h3>
                                <p className="text-xs text-gray-500">Tindakan ini tidak bisa dibatalkan.</p>
                            </div>
                        </div>
                        <p className="text-sm text-gray-700">
                            Apakah Anda yakin ingin menghapus <strong>{deleteTarget.judul}</strong>?
                        </p>
