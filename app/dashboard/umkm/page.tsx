                                    ...
                                    {items.map((u) => (
                                        <tr key={u.id} className="hover:bg-gray-50/70 transition-colors">
                                            <td className="py-3 px-4">
                                                <Link href={`/dashboard/umkm/${u.id}`} className="group flex items-center gap-3">
                                                    {u.gambar ? (
                                                        // eslint-disable-next-line @next/next/no-img-element
                                                        <img src={u.gambar} alt="" className="w-12 h-9 rounded-lg object-cover border border-gray-200 shrink-0" />
                                                    ) : (
                                                        <div className="w-12 h-9 rounded-lg bg-amber-50 flex items-center justify-center shrink-0">
                                                            <ShoppingBag className="w-4 h-4 text-amber-300" />
                                                        </div>
                                                    )}
                                                    <div className="min-w-0">
                                                        <p className="font-semibold text-gray-900 line-clamp-1 group-hover:text-teal-600 transition-colors">{u.nama_usaha}</p>
                                                        {u.deskripsi && <p className="text-[11px] text-gray-400 line-clamp-1 mt-0.5">{u.deskripsi}</p>}
                                                    </div>
                                                </Link>
                                            </td>
                                            ...
                                            <td className="py-3 px-4 text-xs text-gray-400">
                                                {(u.galeri_foto?.length ?? 0) > 0 ? `${u.galeri_foto!.length} galeri` : "—"}
                                            </td>
                                            <td className="py-3 px-4">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Link href={`/dashboard/umkm/${u.id}`} className="p-2 rounded-lg text-gray-400 hover:text-sky-600 hover:bg-sky-50 transition-colors" title="Edit">
